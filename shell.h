...
/**
 * struct passinfo - allows for a standard prototyp
 * @arg: an argument-containing string produced by getline
 * @argv:a collection of strings produced from arg
 ...
 * @environ: a customized version of the environment taken from LL env
 * @history refers to the history node.
 * @alias: the node for the alias If the environment has changed
 * @status: the last command that was executed's return status
 * If chaining, @cmd_buf is the address of the pointer to the cmd_buf.
 * @ CMD_type ||, &&, ; @cmd_buf_type
 * @readfd: the file directory to read lines from
 * @env_changed: if the environment had altered.
...
/* these functions are used to handle command execution */
int hsh(info_t *, char **);
